...
function intro_prof_equivalencies_gp($exportsagaxml,$exporthorarixml)
    ...
    // Eliminem la coletilla profe_actual del camp altres que identifa els professor d'un curs concret
    $sql=" UPDATE equivalencies SET altres = NULL WHERE altres = ' profe_actual';";
    ...
    if ((!$resultatconsulta )AND (extreu_fase('app_horaris')!=4)) // no es carrega l'xml si es tracta d'un csv
      ...
      if((extreu_fase('app_horaris')==0) && (extreu_fase('professorat')==0))
         ...
         foreach ($resultatconsulta->teachers->teacher as $professorgp)
            ...
            if ($resultat[0] == 0)
               {
               $sql="INSERT INTO equivalencies(nom_prof_gp,codi_prof_gp,altres) VALUES('".$professorgp->surname."','".$professorgp['id']."','profe_actual');";
               }
            else 
               {
               $sql="UPDATE equivalencies SET altres = 'profe_actual' WHERE codi_prof_gp='".$professorgp['id']."';";
               }
            $result=mysql_query($sql); if (!$result) {die(_ENPARELLAMENTS_GP_.mysql_error());} 
            }
         ...
         foreach ($resultatconsulta->profesores->profesor as $professorgp)
            ...
            if ($resultat[0] == 0)
               {
               $sql="INSERT INTO equivalencies(nom_prof_gp,codi_prof_gp,altres) VALUES('".$professorgp->nombreCompleto."','".$abreviatura."','profe_actual');";
               }
            else 
               {
               $sql="UPDATE equivalencies SET altres = 'profe_actual' WHERE codi_prof_gp='".$abreviatura."';";
               }
            $result=mysql_query($sql); if (!$result) {die(_ENPARELLAMENTS_GP_.mysql_error());}            
            }
         }
         ...
         foreach ($resultatconsulta->PROFT->PROFF as $professorgp)
            ...
            if ($resultat[0] == 0)
               {
               $sql="INSERT INTO equivalencies(nom_prof_gp,codi_prof_gp,altres) VALUES('".$professorgp['NOMBRE']."','".$abreviatura."','profe_actual');";
               }
            else 
               {
               $sql="UPDATE equivalencies SET altres = 'profe_actual' WHERE codi_prof_gp='".$abreviatura."';";
         ...
         foreach ($professorat as $professorgp)
            {
            $sql="SELECT COUNT(codi_prof_gp) FROM equivalencies WHERE codi_prof_gp='".$professorgp."';";
            $result=mysql_query($sql); if (!$result) {die(_ENPARELLAMENTS_GP2_.mysql_error());}
            $resultat = mysql_fetch_row($result);
            if ($resultat[0] == 0)
               {
                $sql="INSERT INTO equivalencies(nom_prof_gp,codi_prof_gp,altres) VALUES('".$professorgp."','".$professorgp."','profe_actual');";               
                }
            else 
               {
               $sql="UPDATE equivalencies SET altres = 'profe_actual' WHERE codi_prof_gp='".$professorgp."';";
               }
            $result=mysql_query($sql); if (!$result) {die(_ENPARELLAMENTS_GP_.mysql_error());}          

            }
        
        //Esborrem professors d'altre anys que no estan enguany    
        $sql=" SELECT * FROM equivalencies WHERE nom_prof_gp is not NULL AND codi_prof_gp is not NULL AND prof_ga is not NULL AND altres is NULL";
        $result=mysql_query($sql); if (!$result) {die(_ENPARELLAMENTS_GP2_.mysql_error());}            
...
function select_professorat_reserva($exportsagaxml,$exporthorarixml)
	{
...
function select_professoratNomesSAGA($exportsagaxml,$exporthorarixml)
	{
...
function select_professorat($exportsagaxml,$exporthorarixml)
    ...
    // Passem a historic tot el professor no activat
    ...
    //echo extreu_fase('app_horaris');
    $resultatconsulta=simplexml_load_file($exporthorarixml);
    if ( !$resultatconsulta ) {echo "Carrega fallida Horaris >> ".$exporthorarixml;}
    else 
        {
        $columnes=3;
        echo "<br>Càrrega  correcta !";
        print("<table align=\"center\">");
        print("<tr align = \"center\"><td colspan=\"".$columnes."\"><h2>Selecció de professorat</h2><br><font color=\"black\">S'esbrinarà quin professorat encara no ha estat donat d'alta o té les dades incompletes . ");
        print("<br><font color=\"red\"><b>És important</b></font> que seleccionis de la darrera columna, el professor corresponent per tal de fer l'equivalència<br> i que marquis el checkbox per crear l'usuari en el programa d'assistència.</font></td></tr>");
        print("<tr><td></td><td></td><td></td><td><td>Usuaris gassist</td>");
        ...
        // Treiem tots els professors de la taula d'equivalències
        $sql="SELECT codi_prof_gp, nom_prof_gp, prof_ga FROM equivalencies WHERE codi_prof_gp is not NULL AND nom_prof_gp is not NULL ORDER BY nom_prof_gp;";
        $result=mysql_query($sql); if (!$result) {die(_LIST_GP_PROF_.mysql_error());}			
        while ($fila=mysql_fetch_row($result))
            {
            $present = false;
            print("<td>".$fila[0]."</td><td>".$fila[1]."</td>");
        
            // Extreiem els ids del professorat de Gassist que a final de curs estàven actius
            print("<td><select name=\"id_ga".$pos."\" ");
            print(">");
            print("<option value=\"\">No hi ha equivalència</option>");

            $sql3 = "SELECT A.idprofessors FROM professors A,contacte_professor B ";
            $sql3.= "WHERE ((A.idprofessors=B.id_professor) AND (A.historic = 'N'));";
            $result3=mysql_query($sql3); if (!$result3) {die(_LIST_GA_PROF_.mysql_error());}			
            while ($fila3=mysql_fetch_row($result3))
                {
                $sql2 = "SELECT Valor FROM contacte_professor WHERE id_professor = ".$fila3[0]." AND id_tipus_contacte = 1;";
                $result2=mysql_query($sql2); if (!$result2) {die(_LIST_GA_PROF_NAME_.mysql_error());}			
                while ($fila2=mysql_fetch_row($result2))
                    {
                    print("<option value=\"".$fila[0]."\" ");
                    if ($fila3[0] == $fila[0]) {print(" SELECTED ");$present = true;}
                    print(">".$fila2[0]."</option>");
                    }
                }    
            print("</select></td>");
            
            // Si no l'ha reconegut posa el check box per crear-lo
            print("<td><input type=\"checkbox\" name=\"alta".$pos."\" ");
            if (!extreu_fase('geisoft'))
                {
                if (extreu_fase('carrega')==0) {print("CHECKED ");}
                }
            else 
                {
                //Està desactivat si utilitzo gestio centralitzada
        ...
        print("</tr> ");
            ...
            // Mirem si ja té l'id de SAGA assignat
             AND (B.id_tipus_contacte='".$camps['iden_ref']."') ";
            $sql.=" AND (B.Valor='".$professor['id']."'));";//echo $sql;
            $result=mysql_query($sql);
            if (!$result) {die(_ERR_LOOK_FOR_PROF . mysql_error());}
            $fila_prof=  mysql_fetch_row($result);
