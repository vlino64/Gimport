    ...
    for ($i = 1 ; $i < count($alumnat); $i++)
        ...
        for ($j = 0 ; $j < 3 ; $j++)
            ...
            foreach ($grups as $grup ) 
                {
                if (!strcmp($grup,$grupTmp)) {$present = true; break;}
                }
            if (!$present) 
                {
                $grups[$k] = $grupTmp; 
...
function emparella_grups_actualitzacio_csv()
    ...
    print("<tr align=\"center\" bgcolor=\"#ffbf6d\" ><td>Grup csv </td><td></td><td>Grups app horaris</td></tr>");
    $pos=1;
    //echo "<br>".$sql;
...
function _comprova_matricula_grup($idAlumne,$id_grup){
    ...
    if (!$result) {die(_ERR_SELECT_GMAT . mysql_error());}
    $present = 0;
    while (($fila=mysql_fetch_row($result)) && ($present == 0))
        {
		$sql2 = "SELECT COUNT(idalumnes_grup_materia) FROM alumnes_grup_materia WHERE idalumnes = ".$idAlumne." AND ";
		$sql2.= "idgrups_materies = ".$fila[0].";";	
		$result2=mysql_query($sql2);
		if (!$result2) {die(_ERR_COUNT1 . mysql_error());}	
		$fila2=mysql_fetch_row($result2);$present = $fila2[0];
		}
	if ($present == 0) {alta_grup_actualitzacio($idAlumne,$id_grup);}
	}
        ...
        if ($idAlumne != "")
            ...
            if (alumne_ja_existeix($idAlumne))
                ...
                if ($grup1 != "")
                    ...
                    comprova_matricula_grup($id,$id_grup);
                    }  
                if ($grup2 != "")
                    {
                ...
                if (!$result) 
                        {
                        die(_ERR_INSERT_ALUM_CONTACT3 . mysql_error());
                        }
...
//                    echo"<br>".$id_grup." >> ".$grup1." >> ".$id;
                    if ($id_grup != 0 ) {alta_grup_actualitzacio($id,$id_grup);}
                    }  
                if ($grup2 != "")
                    {
                    $id_grup = extreuGrup($relacioGrups,$grup2);
...
function _extreuGrup($relacioGrups,$grup)
    ...
    foreach ($relacioGrups as $grups)
        ...
        if ($grups[2] == $grup) {return $grups[0];}
        }
    return 0;
    }
...
//        echo "<br>".$sql2;
        ...
        if (!$result2) 
            {
            die(_ERR_SELECT_GRUP_MAT2 . mysql_error());
            }
        }
    }
