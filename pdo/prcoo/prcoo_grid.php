...
?>
    ...
    <table id="dg" class="easyui-datagrid" style="height:540px;" 
        url="./prcoo/prcoo_getdata.php"  
        title="Hores de coordinaci&oacute; professors" toolbar="#toolbar" fitColumns="false"
    ...
    <script type="text/javascript">  
		...
		$('#dg').datagrid({  
			...
			onExpandRow: function(index,row){  
				$('#ddv-'+index).datagrid({  
					url:'./prcoo/prcoo_getdetail.php?idprofessors='+row.id_professor,  
					fitColumns:false,  
					rownumbers:true,  
					loadMsg:'hores coordinacio del professor...',  
					height:'auto',
					columns:[[  
		...
		function endEditing(){
			...
			if ($('#dg_mat').datagrid('validateRow', editIndex)){
				...
				if (nou_registre) { 
					url = './prcoo/prcoo_nou.php';
					nou_registre = 0;
				}
				else {
					url = './prcoo/prcoo_edita.php?id='+$('#dg_mat').datagrid('getRows')[editIndex]['idprofcoordinacio'];
				}
				afterEdit(url,
						  row_p.id_professor,
		...
		function accept(){			
			if (endEditing()){
				...
				if (nou_registre) { 
					...
					url = './prcoo/prcoo_edita.php?id='+row_a.idprofcoordinacio;
				}
		...
		function destroyItem(){  
            ...
            if (row_a){  
                $.messager.confirm('Confirmar','Est&aacute;s segur de que vols esborrar?',function(r){  
                    if (r){  
                        $.post('./prcoo/prcoo_esborra.php',{id:row_a.idprofcoordinacio},function(result){  
                            if (result.success){  
