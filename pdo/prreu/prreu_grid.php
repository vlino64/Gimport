...
?>
    ...
    <table id="dg" class="easyui-datagrid" style="height:540px;" 
        url="./prreu/prreu_getdata.php"  
        title="Hores de reunions professors" toolbar="#toolbar" fitColumns="false"
    ...
    <script type="text/javascript">  
		...
		$('#dg').datagrid({  
			...
			onExpandRow: function(index,row){  
				$('#ddv-'+index).datagrid({  
					url:'./prreu/prreu_getdetail.php?idprofessors='+row.id_professor,  
					fitColumns:false,  
					rownumbers:true,  
					loadMsg:'hores reunions del professor...',  
					height:'auto',
					columns:[[  
		...
		function endEditing(){
			...
			if ($('#dg_mat').datagrid('validateRow', editIndex)){
				...
				if (nou_registre) { 
					url = './prreu/prreu_nou.php';
					nou_registre = 0;
				}
				else {
					url = './prreu/prreu_edita.php?id='+$('#dg_mat').datagrid('getRows')[editIndex]['idprofreunio'];
				}
				afterEdit(url,
						  row_p.id_professor,
		...
		function accept(){			
			if (endEditing()){
				...
				if (nou_registre) { 
					...
					url = './prreu/prreu_edita.php?id='+row_a.idprofreunio;
				}
		...
		function destroyItem(){  
            ...
            if (row_a){  
                $.messager.confirm('Confirmar','Est&aacute;s segur de que vols esborrar?',function(r){  
                    if (r){  
                        $.post('./prreu/prreu_esborra.php',{id:row_a.idprofreunio},function(result){  
                            if (result.success){  
