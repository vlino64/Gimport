...
$sql  = "SELECT uc.*,CONCAT(LEFT(fh.hora_inici,5),'-',LEFT(fh.hora_fi,5)) AS dia_hora,m.nom_materia,ec.descripcio,g.nom as grup,fh.idfranges_horaries ";
$sql .= "FROM unitats_classe uc ";
$sql .= "INNER JOIN alumnes_grup_materia   agm ON uc.idgrups_materies   = agm.idgrups_materies ";
...
$sql .= "WHERE fh.esbarjo<>'S' AND ds.iddies_setmana=".$dia_setmana." AND df.idperiode_escolar=".$curs_actual." AND agm.idalumnes=".$idalumne." ";

$sql .= "UNION ";

$sql .= "SELECT uc.*,CONCAT(LEFT(fh.hora_inici,5),'-',LEFT(fh.hora_fi,5)) AS dia_hora,CONCAT(m.nom_modul,'-',uf.nom_uf) AS nom_materia,ec.descripcio,g.nom as grup,fh.idfranges_horaries ";
$sql .= "FROM unitats_classe uc ";
$sql .= "INNER JOIN alumnes_grup_materia   agm ON uc.idgrups_materies   = agm.idgrups_materies ";
$sql .= "INNER JOIN dies_franges     df ON uc.id_dies_franges    = df.id_dies_franges ";
$sql .= "INNER JOIN dies_setmana     ds ON df.iddies_setmana     = ds.iddies_setmana ";
