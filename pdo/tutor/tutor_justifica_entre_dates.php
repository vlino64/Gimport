			...
			$rsMateriaAlumne    = getMateriesDiaHoraAlumne($db,date('w'),$idfranges_horaries,$curs,$idalumnes);    
                            ...
                            foreach($rsMateriaAlumne->fetchAll() as $row_mat) {
                                ...
                                foreach($rsProfMateria->fetchAll() as $row_pm) {
                                    ...
                                    if ($absencia && existsIncidenciaAlumnebyTipus($db,$idalumnes,$data_actual,$idfranges_horaries,TIPUS_FALTA_ALUMNE_ABSENCIA)) {
                                    ...
                                    if ($retard && existsIncidenciaAlumnebyTipus($db,$idalumnes,$data_actual,$idfranges_horaries,TIPUS_FALTA_ALUMNE_RETARD)) {
                                            $sql  = "DELETE FROM incidencia_alumne ";
                                            $sql .= "WHERE idalumnes=".$idalumnes." AND idfranges_horaries=".$idfranges_horaries;
                                            $sql .= " AND data='".$data_actual."' AND id_tipus_incidencia=".TIPUS_FALTA_ALUMNE_JUSTIFICADA;
                                            $result = $db->query($sql);
                                    ...
                                    if ($seguiment && existsIncidenciaAlumnebyTipus($db,$idalumnes,$data_actual,$idfranges_horaries,TIPUS_FALTA_ALUMNE_SEGUIMENT)) {
                                            $sql  = "DELETE FROM incidencia_alumne ";
                                            $sql .= "WHERE idalumnes=".$idalumnes." AND idfranges_horaries=".$idfranges_horaries;
                                            $sql .= " AND data='".$data_actual."' AND id_tipus_incidencia=".TIPUS_FALTA_ALUMNE_JUSTIFICADA;
                                            $result = $db->query($sql);
