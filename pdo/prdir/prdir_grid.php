...
?>
    ...
    <table id="dg" class="easyui-datagrid" style="height:540px;" 
        url="./prdir/prdir_getdata.php"  
        title="Hores de direcci&oacute; professors" toolbar="#toolbar" fitColumns="false"
    ...
    <script type="text/javascript">  
		...
		$('#dg').datagrid({  
			...
			onExpandRow: function(index,row){  
				$('#ddv-'+index).datagrid({  
					url:'./prdir/prdir_getdetail.php?idprofessors='+row.id_professor,  
					fitColumns:false,  
					rownumbers:true,  
					loadMsg:'hores direccio del professor...',  
					height:'auto',
					columns:[[  
		...
		function endEditing(){
			...
			if ($('#dg_mat').datagrid('validateRow', editIndex)){
				...
				if (nou_registre) { 
					url = './prdir/prdir_nou.php';
					nou_registre = 0;
				}
				else {
					url = './prdir/prdir_edita.php?id='+$('#dg_mat').datagrid('getRows')[editIndex]['idprofdireccio'];
				}
				afterEdit(url,
						  row_p.id_professor,
		...
		function accept(){			
			if (endEditing()){
				...
				if (nou_registre) { 
					...
					url = './prdir/prdir_edita.php?id='+row_a.idprofdireccio;
				}
		...
		function destroyItem(){  
            ...
            if (row_a){  
                $.messager.confirm('Confirmar','Est&aacute;s segur de que vols esborrar?',function(r){  
                    if (r){  
                        $.post('./prdir/prdir_esborra.php',{id:row_a.idprofdireccio},function(result){  
                            if (result.success){  
