...
?>        
    ...
    <table id="dg" class="easyui-datagrid" title="Sortides enregistrades administraci&oacute;" style="height:540px;"
	data-options="
		singleSelect: true,
                pagination: true,
                rownumbers: true,
		toolbar: '#toolbar',
		url: './sortides_adm/sortides_adm_getdata.php',
	...
	">    
        <thead>  
            <tr>
                ...
                <th sortable="true" data-options="field:'lloc',width:180,align:'left',editor:{type:'validatebox',validType:'length[0,55]',required:true}">Lloc</th>
                <th sortable="true" data-options="field:'descripcio',width:430,editor:{type:'textarea',validType:'length[0,256]'}">Descripci&oacute;</th>
            </tr>  
        </thead>  
    </table> 
    ...
    <div id="toolbar" style="padding:5px;height:auto">  
        ...
        &nbsp;&nbsp;&nbsp;&nbsp;
        <a id="esborrar_sortida" href="javascript:void(0)" class="easyui-linkbutton" data-options="iconCls:'icon-cancel',plain:true,disabled:true" onclick="esborrarSortida()">Esborrar sortida</a>
    </div>
    </div>
	...
	function myformatter(date){  
                    ...
                    if (r){  
                        $.post(url,{
                            ...
                            if (result.success){  
								   ...
								   $('#esborrar_sortida').linkbutton('disable');
		...
		function esborrarSortida(){ 
		  var row = $('#dg').datagrid('getSelected');
		  
		  if (row){ 
			   url = './sortides_adm/sortides_adm_esborra.php';
			   
			   $.messager.confirm('Confirmar','Esborrem aquesta sortida?',function(r){  
                    if (r){  
                        $.post(url,{
								id:row.idsortides},function(result){  
                            if (result.success){  
                                   $.messager.alert('Informaci&oacute;','Sortida esborrada correctament!','info');
								   $('#dg').datagrid('reload');
								   editIndex = undefined;
								   $('#gestio_professors').linkbutton('disable');
								   $('#gestio_alumnes').linkbutton('disable');
							    ...
							    $.messager.alert('Error','Sortida esborrada erroniament!','error');
		...
		function onClickRow(index){
				...
				$('#esborrar_sortida').linkbutton('enable');
