...
?>        
	...
	data-options="
		...
		singleSelect: true,
                ...
                rownumbers: true,
		toolbar: '#tb',
                url: './conserge/conserge_alumnes_getdata.php?sms=0&data=<?=$data?>'
        ">    
        <thead>  
            <tr>
    ...
    <?php include_once('../conserge/conserge_form_email.php'); ?>
    ...
    <script type="text/javascript">  
		...
		function getSelections(){
			var ss = []; var ssm = []; var ssd = []; 
                        var rows = $('#dg_sms').datagrid('getSelections');
                        
                        ss = $("#frm_email textarea[name=destinataris]").val().split('\n');
			...
			var row = rows[i];
			  ss.push(row.idalumnes);
                          ss_acm.push(row.idalumnes);
			  ssm = ssm + (row.alumne+'\n');
			}
                         
			url = './conserge/conserge_set_target_email.php?idalumnes='+ss_acm;
		...
		function doReloadEmail(idgrups,nomgrup){
			url = './conserge/conserge_email.php?idgrups='+idgrups;
			$('#dlg_fm_sms').dialog('refresh', url);
		}
		
		function enviarEmail(idgrups,nomgrup){
		    var rows = $('#dg_sms').datagrid('getSelections');
			if (rows) {  
			    var ss = [];
			    for(var i=0; i<rows.length; i++){
				...
				url = './conserge/conserge_form_email.php?idalumnes='+ss;
				$('#dlg_fm_sms').dialog('open').dialog('setTitle','Enviar Email grup '+nomgrup);
				$('#dlg_fm_sms').dialog('refresh', url);
			}
		}
