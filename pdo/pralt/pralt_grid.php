...
?>
    ...
    <table id="dg" class="easyui-datagrid" style="height:540px;" 
        url="./pralt/pralt_getdata.php"  
        title="Hores d'altres professors" toolbar="#toolbar" fitColumns="false"
	...
	var nou_registre = 0;
		...
		$('#dg').datagrid({  
			...
			onExpandRow: function(index,row){  
				$('#ddv-'+index).datagrid({  
					url:'./pralt/pralt_getdetail.php?idprofessors='+row.id_professor,  
					fitColumns:false,  
					rownumbers:true,  
					loadMsg:'altres hores del professor...',  
					height:'auto',
					columns:[[  
	...
	function imprimirHorario(){
		...
		function endEditing(){
			...
			if ($('#dg_mat').datagrid('validateRow', editIndex)){
				...
				if (nou_registre) { 
					url = './pralt/pralt_nou.php';
					nou_registre = 0;
				}
				else {
					url = './pralt/pralt_edita.php?id='+$('#dg_mat').datagrid('getRows')[editIndex]['idprofreunio'];
				}
				afterEdit(url,
						  row_p.id_professor,
		...
		function accept(){			
			if (endEditing()){
				...
				if (nou_registre) { 
					...
					url = './pralt/pralt_edita.php?id='+row_a.idprofreunio;
				}
	...
	function destroyItem(){  
            ...
            if (row_a){  
                $.messager.confirm('Confirmar','Est&aacute;s segur de que vols esborrar?',function(r){  
                    if (r){  
                        $.post('./pralt/pralt_esborra.php',{id:row_a.idprofreunio},function(result){  
                            if (result.success){  
