...
?>
    ...
    <table id="dg" class="easyui-datagrid" style="height:540px;" 
        url="./prate/prate_getdata.php"  
        title="Hores de atencions professors" toolbar="#toolbar" fitColumns="false"
    ...
    <script type="text/javascript">  
		...
		$('#dg').datagrid({  
			...
			onExpandRow: function(index,row){  
				$('#ddv-'+index).datagrid({  
					url:'./prate/prate_getdetail.php?idprofessors='+row.id_professor,  
					fitColumns:false,  
					rownumbers:true,  
					loadMsg:'hores atencions del professor...',  
					height:'auto',
					columns:[[  
		...
		function endEditing(){
			...
			if ($('#dg_mat').datagrid('validateRow', editIndex)){
				...
				if (nou_registre) { 
					url = './prate/prate_nou.php';
					nou_registre = 0;
				}
				else {
					url = './prate/prate_edita.php?id='+$('#dg_mat').datagrid('getRows')[editIndex]['idprofatencio'];
				}
				afterEdit(url,
						  row_p.id_professor,
		...
		function accept(){			
			if (endEditing()){
				...
				if (nou_registre) { 
					...
					url = './prate/prate_edita.php?id='+row_a.idprofatencio;
				}
		...
		function destroyItem(){  
            ...
            if (row_a){  
                $.messager.confirm('Confirmar','Est&aacute;s segur de que vols esborrar?',function(r){  
                    if (r){  
                        $.post('./prate/prate_esborra.php',{id:row_a.idprofatencio},function(result){  
                            if (result.success){  
