...
//	$sql .= "idprofessors < 440 AND ";
	...
	foreach($result->fetchAll() as $fila) {
		...
		$sql .= "WHERE ";
		$sql .= "idgrups = ".$idGrup.";";
		$result2=$db->query($sql); if (!$result2) {die(_SELECT_NOM_GRUP.mysqli_error($conn));}    
		$fila2=  $result2->fetchAll();
		$nomGrup = $fila2[0]['nom'];    
		...
		$sql .= "id_grups = ".$idGrup.";";
		//echo "<br>".$sql;
		$result2=$db->query($sql); if (!$result2) {die(_SELECT_PROF_GRUP_MAT.mysqli_error($conn));}
		...
		// Per cada grup materia de cada professor, dia a dia  les classes que té
			...
			foreach($result3->fetchAll() as $fila3)  {
				...
				foreach ($arrAlumnat as $alumnes){
					if ($alumnes[0] == $idAlumne) $present = true;
				}
				if (!$present){
				   ...
				   $result4 = $db->query($sql); if (!$result4) {die(_SELECT_DAYS_TIMES.mysqli_error($conn));}
		...
		for ($i = 0 ; $i < $count ; $i++){
			...
			if (($idAlumne != "") && ($nomAlumne != ""))
				...
				$result2 = $db->query($sql); if (!$result2) {die(_SELECT_INC_ALUMNES1.mysqli_error($conn));}
				$fila2=  $result2->fetchAll();
	...
	//            echo "<br>".$arrAlumnat[$i][2];
				...
				$sql .= "idalumnes = ".$idAlumne." AND data >= '".$dataInici."' AND data <= '".$dataFi."';";
				$result2 = $db->query($sql); if (!$result2) {die(_SELECT_INC_ALUMNES2.mysqli_error($conn));}
				$fila2=  $result2->fetchAll();
	...
	//            echo "<br>".$arrAlumnat[$i][3];
				...
				$sql .= "idalumnes = ".$idAlumne." AND data >= '".$dataInici."' AND data <= '".$dataFi."';";
				$result2 = $db->query($sql); if (!$result2) {die(_SELECT_INC_ALUMNES3.mysqli_error($conn));}
				$fila2=  $result2->fetchAll();
	...
	//            echo "<br>".$arrAlumnat[$i][4];
				...
				$sql .= "idalumnes = ".$idAlumne." AND data >= '".$dataInici."' AND data <= '".$dataFi."';";
				$result2 = $db->query($sql); if (!$result2) {die(_SELECT_INC_ALUMNES4.mysqli_error($conn));}
				$fila2=  $result2->fetchAll();
	...
	//            echo "<br>".$arrAlumnat[$i][5];
				...
				$result2 = $db->query($sql); if (!$result2) {die(_SELECT_INC_ALUMNES5.mysqli_error($conn));}
				$fila2=  $result2->fetchAll();
				$arrAlumnat[$i][6] = $fila2[0]['COUNT(idincidencia_alumne)'];         
				}
			
			
		}

		include('./cron_tutor_assistencia_send.php');
