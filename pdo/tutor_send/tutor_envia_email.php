...
foreach ( $destinataris as $idalumne ) {
	...
	if (trim($email_tutor1 != '')) {
	  mail($email_tutor1,$subject,$contents.$footer,$header);
          ...
          // Afegim entrada al fitxer de log
          $sql  = "INSERT INTO log_correus (data,hora,idprofessor,idalumne,missatge) VALUES ('";
          $sql .= date("y-m-d")."','".date("H:i")."',".$idprofessor.",".$idalumne.",'".$contents."')";
          $result = $db->query($sql);
