<?php
	...
	function getHorariAlumne($idAlumne,$curs,$cursLiteral){
		...
		$nom_alumne = getAlumne($db,$idAlumne,TIPUS_nom_complet);
		...
		for($dia=1; $dia<=5; $dia++){
			...
			while($row = mysql_fetch_object($rsHores)){
				...
				if ($row["esbarjo"]=='S') {
					...
					$rsMateries = getMateriesDiaHoraAlumne($db,$dia,$franjahoraria,$curs,$idAlumne);
					foreach($rsMateries->fetchAll() as $row) {
						...
						if(!is_object(getProfessorByGrupMateria($db,$row['idgrups_materies']))) {
							$nom_professor = " ";
						}else {
							$nom_professor = utf8_encode(getProfessor($db,getProfessorByGrupMateria($db,$row['idgrups_materies'])["idprofessors"],TIPUS_nom_complet));
						}
	...
	function getAssistencia($idAlumne){
		...
		if(mysql_num_rows($rsIncidencias) > 0){
			...
			while($row = mysql_fetch_object($rsIncidencias)){
				...
				$retorno['faltes'][$linea]['professor'] = utf8_encode(getProfessor($db,$row["idprofessors"],TIPUS_nom_complet));
				$retorno['faltes'][$linea]['materia'] = utf8_encode(getMateria($db,$row["id_mat_uf_pla"])["nom_materia"]);
				$linea++;
			}
		}else{
			$retorno['faltes'] = "";
		...
		if(mysql_num_rows($rsIncidencias) > 0){
			...
			while($row = mysql_fetch_object($rsIncidencias)){
				...
				$retorno['retards'][$linea]['professor'] = utf8_encode(getProfessor($db,$row["idprofessors"],TIPUS_nom_complet));
				$retorno['retards'][$linea]['materia'] = utf8_encode(getMateria($db,$row["id_mat_uf_pla"])["nom_materia"]);
				$linea++;
			}
		}else{
			$retorno['retards'] = "";
		...
		if(mysql_num_rows($rsIncidencias) > 0){
			...
			while($row = mysql_fetch_object($rsIncidencias)){
				...
				$retorno['justificacions'][$linea]['professor'] = utf8_encode(getProfessor($db,$row["idprofessors"],TIPUS_nom_complet));
				$retorno['justificacions'][$linea]['materia'] = utf8_encode(getMateria($db,$row["id_mat_uf_pla"])["nom_materia"]);
		...
		$rsIncidencias = getIncidenciasAlumne($db,$idAlumne,TIPUS_FALTA_ALUMNE_INCIDENT,$data_inici,$data_fi);
		//error_log($idAlumne.":".mysql_num_rows($rsIncidencias));
		if(mysql_num_rows($rsIncidencias) > 0){
			...
			while($row = mysql_fetch_object($rsIncidencias)){
				...
				$retorno['incidencies'][$linea]['tipus'] = utf8_encode(getLiteralTipusIncident($db,$row->id_tipus_incident)->tipus_incident);
				$retorno['incidencies'][$linea]['professor'] = utf8_encode(getProfessor($db,$row["idprofessors"],TIPUS_nom_complet));
				$retorno['incidencies'][$linea]['materia'] = utf8_encode(getMateria($db,$row["id_mat_uf_pla"])["nom_materia"]);
		...
		if(mysql_num_rows($rsIncidencias) > 0){
			...
			while($row = mysql_fetch_object($rsIncidencias)){
				...
				$retorno['ccc'][$linea]['tipus'] = utf8_encode(getLiteralTipusCCC($db,$row->id_falta)->nom_falta);
				$retorno['ccc'][$linea]['expulsio'] = utf8_encode($row->expulsio);
				$retorno['ccc'][$linea]['professor'] = utf8_encode(getProfessor($db,$row["idprofessors"],TIPUS_nom_complet));
				$retorno['ccc'][$linea]['materia'] = utf8_encode((intval($row->idmateria!=0) ? getMateria($db,$row->idmateria)["nom_materia"] : ''));
	...
	function getMensajeria($idAlumne){
		...
		$idprofessor   = 0;
		$nom_tutor     = "";
		if(is_object(getIdProfessorByCarrecGrup($db,$idTutorCarrec,$idgrup))){
			$idprofessor   = utf8_encode(getIdProfessorByCarrecGrup($db,$idTutorCarrec,$idgrup)["idprofessors"]);
			$nom_tutor     = utf8_encode(getProfessor($db,$idprofessor,TIPUS_nom_complet));
		}
