...
<?php
		...
		for ($i=1;$i<=$recompte;$i++)
			...
			// Aprofito per comptar els checkbox de " No està present a la llista" que hi haurà
			// per tal de mostrar o no el llistat dels històrics
			//$missing="no_ap".$i;
			//echo $missing."<br>";
			//if($_POST[$missing]) {$noap++;}
...
//			$actualitzacio="actualitzacio".$i;
//			//echo $actualitzacio;
//			if($_POST[$actualitzacio]) 
//				{
//				
//				$sql="UPDATE `alumnes` SET `codi_alumnes_saga`='".$id_saga."',activat='S' WHERE idalumnes=\"".$id_gassist."\";";
//				$result=mysql_query($sql);
//				//echo $sql;
//				if (!$result) 
//					{
//					die(_ERR_UPDATE_ALUM_SAGA_ID . mysql_error());
//					}
//				print("Alumne/a actualitzat: ".$nom." ".$cognom1." ".$cognom2." <br>");
//				}
